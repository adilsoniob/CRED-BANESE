        ...
        <AnimatePresence>
          {isSecurityPopupOpen && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
              <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} transition={{ type: 'spring', damping: 25, stiffness: 300 }} className="w-full max-w-[420px] bg-white rounded-3xl shadow-2xl border border-slate-100 overflow-hidden">
                <div className="flex items-center justify-between px-5 pt-5 pb-0">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-emerald-900 to-emerald-700 flex items-center justify-center text-white text-[6px] font-black">B·CV</div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => setIsSecurityPopupOpen(false)} className="w-7 h-7 flex items-center justify-center rounded-full bg-slate-100 hover:bg-slate-200 text-slate-400 hover:text-slate-600 transition-colors"><X className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
                <div className="px-5 pt-3 pb-5">
                  <h3 className="font-display font-extrabold text-base text-[#1F2937] mb-3">🔒 Aviso Importante</h3>
                  <div className="text-xs text-[#4B5563] leading-relaxed space-y-2.5">
                    <p className="font-extrabold text-emerald-700 text-sm bg-emerald-50 -mx-1 px-3 py-2.5 rounded-xl border border-emerald-100">A CredVale <span className="text-[#DC2626]">NÃO</span> cobra qualquer valor antecipado.</p>
                    <p>A CredVale NÃO cobra qualquer valor antecipado para análise, aprovação ou liberação de crédito.</p>
                    <p>Não solicitamos depósitos, PIX, transferências ou qualquer pagamento antecipado para liberar limites ou concluir cadastros.</p>
                    <p>Se alguém pedir qualquer valor em nome da CredVale antes da contratação, desconsidere e entre em contato.</p>
                    <p className="font-semibold text-[#1F2937] pt-1">Sua segurança e transparência são prioridades para nós.</p>
                  </div>
                  <button onClick={() => setIsSecurityPopupOpen(false)} className="mt-4 w-full bg-emerald-600 hover:bg-emerald-500 text-white font-bold py-3 rounded-xl transition-all active:scale-95 duration-200 text-sm">Entendi</button>
                </div>
              </motion.div>
            </div>
          )}
